...
export class GamifyCollaboration {
  ...
  constructor(token: string, excalidrawAPI: ExcalidrawImperativeAPI) {
    this.token = token;
  ...
  public start = (boardId: string) => {
    const wsUrl =
      import.meta.env.VITE_APP_COLLAB_URL || "ws://localhost:1234";
    ...
    this.socket.on("connect", () => {
      ...
      console.log(
        "Socket connected, joining board via custom event 'yjs:join'...",
      );
    ...
    this.socket.on(
      "yjs:update",
      (message: { traceId: string; payload: Uint8Array }) => {
        const update = new Uint8Array(message.payload); // NEU: Payload extrahieren
        console.log(
          `[CLIENT] [${message.traceId}] Received document update from server. Size: ${update.byteLength} bytes.`,
        ); // NEU: Trace-ID loggen
    ...
    this.ydoc.on("update", (update: Uint8Array, origin: any) => {
      if (origin !== this) {
        // 'this' is the origin for remote changes
        const traceId = Math.random().toString(36).substring(2, 15); // NEU: Trace-ID generieren
        console.log(
          `[CLIENT] [${traceId}] Sending document update to server. Size: ${update.byteLength} bytes.`,
        );
        this.socket?.emit("yjs:update", { traceId, payload: update }); // NEU: Objekt senden
      }
    });
    ...
    this.socket.on(
      ...
      (message: { traceId: string; payload: Uint8Array }) => {
        const update = new Uint8Array(message.payload); // NEU: Payload extrahieren
        console.log(
          `[CLIENT] [${message.traceId}] Received awareness update from server.`,
        ); // NEU: Trace-ID loggen
    ...
    this.awareness.on("update", (changes: any, origin: any) => {
      if (origin === "local") {
        ...
        const traceId = Math.random().toString(36).substring(2, 15); // NEU: Trace-ID generieren
        console.log(
          `[CLIENT] [${traceId}] Sending awareness update to server.`,
        );

...
const AuthPanel = React.forwardRef<  HTMLDivElement,  {    authPanelView: "login" | "register";    setAuthPanelView: (view: "login" | "register" | null) => void;    onLoginSuccess: () => void;  }>(({ authPanelView, setAuthPanelView, onLoginSuccess }, ref) => {
    const { t } = useI18n();
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setError(
          t(isLoginView ? "auth.loginFailed" : "auth.registrationFailed", {
            details: err.message || "",
          }),
        );
      }
    ...
    const mainDomainUrl =
      import.meta.env.VITE_APP_MAIN_DOMAIN_URL || "https://gamifyboard.com";
          ...
          <div className="Modal__switch">
            {isLoginView ? (
              <p>
                ...
                <br />
                <a
                  href={`${mainDomainUrl}/reset-password`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="Modal__switch-link"
                >
